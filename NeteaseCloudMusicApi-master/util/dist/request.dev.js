...
var createRequest = function createRequest(method, url, data, options) {
  return new Promise(function (resolve, reject) {
    ...
    if (options.realIP) headers['X-Real-IP'] = options.realIP; // headers['X-Real-IP'] = '118.88.88.88'

    if (_typeof(options.cookie) === 'object') headers['Cookie'] = Object.keys(options.cookie).map(function (key) {
      return encodeURIComponent(key) + '=' + encodeURIComponent(options.cookie[key]);
    }).join('; ');else if (options.cookie) headers['Cookie'] = options.cookie;
    ...
    if (options.crypto === 'weapi') {
      ...
      headers['Cookie'] = Object.keys(header).map(function (key) {
        return encodeURIComponent(key) + '=' + encodeURIComponent(header[key]);
      }).join('; ');
      data.header = header;
    ...
    request(settings, function (err, res, body) {
      if (err) {
        ...
        answer.cookie = (res.headers['set-cookie'] || []).map(function (x) {
          return x.replace(/\s*Domain=[^(;|$)]+;*/, '');
        });

        try {
          if (options.crypto === 'eapi') {
            zlib.unzip(body, function (err, buffer) {
              var _buffer = err ? body : buffer;
              ...
              if (answer.status === 200) resolve(answer);else reject(answer);
            });
            return false;
          } else {
